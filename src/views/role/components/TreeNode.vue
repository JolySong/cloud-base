    ...
    <div class="node-content">
      <div class="node-row" :style="{ paddingLeft: level * 24 + 'px' }">
        ...
        <label class="checkbox-wrapper">
          ...
          >
          <span :class="['resource-tag', 'tag-' + getResourceTypeClass(node.resourceType)]">
            {{ getResourceTypeLabel(node.resourceType) }}
          </span>
          <span class="node-label">{{ node.resourceName }}</span>
          <span v-if="node.apiUrl" class="resource-path">
            ({{ node.apiUrl }})
          </span>
          <span v-else class="resource-path">
            ({{ node.resourceKey }})
          </span>
        </label>
      </div>
